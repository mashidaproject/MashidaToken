  ...
  , ReentrancyGuard
{
  ...
  string constant private _symbol = "MSI";
  ...
  //split TGE Token 
  address constant PRESALE = 0xaCd02398e94F07Ccb3907252E1ce2665843f573f; //change to real address
  address constant CEXALLOCATION = 0x7576A4826B11E15c086cf52A962F677cAa73692C; //change to real address
  ...
  address constant DEVELOPMENT = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4; //change to real address
  ...
  constructor() {
    ...
    _mint(CEXALLOCATION, 300000000 * 10 ** uint256(_decimals)); //30%
    _mint(ECOSYSTEM, 250000000 * 10 ** uint256(_decimals)); //25%
    _mint(TEAM, 100000000 * 10 ** uint256(_decimals)); //10%
    _mint(MARKETING, 100000000 * 10 ** uint256(_decimals)); //10%
    _mint(DEVELOPMENT, 200000000 * 10 ** uint256(_decimals)); //20%
  }
